...
export default function InstructorDashboard() {
  ...
  const students: any[] = useMemo(() => studentsResp?.data?.students ?? [], [studentsResp])
  ...
  const sectionsAgg = useMemo(() => {
    const map = new Map<string, { id: string; name: string; totalStudents: number; attendancePctSum: number; attendancePctCount: number; approvedReqs: number; totalReqs: number }>()
    ...
    for (const student of students) {
      ...
      const presentish = records.filter((r) => ["present", "late", "excused"].includes(r.status)).length
      if (totalDays > 0) {
    ...
    return Array.from(map.values()).map((b) => {
      const avgAttendance = b.attendancePctCount > 0 ? Math.round(b.attendancePctSum / b.attendancePctCount) : 0
      const completionRate = b.totalReqs > 0 ? Math.round((b.approvedReqs / b.totalReqs) * 100) : 0
      return {
        id: b.id,
        name: b.name,
  ...
  const stats = useMemo(() => {
    ...
    for (const student of students) {
      ...
      totalPresentish += records.filter((r) => ["present", "late", "excused"].includes(r.status)).length
    }
    const averageAttendance = totalRecords > 0 ? Math.round((totalPresentish / totalRecords) * 100) : 0
  ...
  const weeklyAttendanceData = useMemo(() => {
    ...
    const now = new Date()
    const weeks: { weekStart: string; attendance: number; submissions: number }[] = []
    ...
    for (const student of students) {
      ...
      for (const r of records) {
        ...
        b.total += 1
        if (["present", "late", "excused"].includes(r.status)) b.presentish += 1
      }
    }
    ...
    for (let i = 7; i >= 0; i--) {
      ...
      const attendance = agg && agg.total > 0 ? Math.round((agg.presentish / agg.total) * 100) : 0
      weeks.push({ weekStart: wk, attendance, submissions: 0 })
    }

    return weeks.map((w, idx) => ({ week: `W${idx + 1}`, attendance: w.attendance, submissions: w.submissions }))
  }, [students])

  const sectionPerformanceData = useMemo(() => {
    return sectionsAgg.map((s) => ({ section: s.name, completion: s.completionRate }))
  }, [sectionsAgg])
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Section */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Welcome back{user?.firstName ? `, ${user.firstName}!` : "!"}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="bg-secondary-50 border-primary-200">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-gray-900">{isUserLoading || isStudentsLoading ? "…" : stats.totalStudents}</div>
            <p className="text-sm text-gray-600">Across {isUserLoading || isStudentsLoading ? "…" : stats.sectionsCount} sections</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-secondary-50 border-accent-200">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-gray-900">{isUserLoading || isStudentsLoading ? "…" : `${stats.averageCompletion}%`}</div>
        ...
        <Card className="bg-secondary-50 border-green-200">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-gray-900">{isUserLoading || isStudentsLoading ? "…" : `${stats.averageAttendance}%`}</div>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {isAttendanceLoading ? "Loading..." : `${pendingItems.attendance.length} pending approvals`}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {isReportsLoading ? "Loading..." : `${pendingItems.reports.length} new submissions`}
                </p>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {isRequirementsLoading ? "Loading..." : `${pendingItems.requirements.length} documents ready for review`}
                </p>
              </div>
              <div className="flex items-center gap-2">
