...
import { Course, CourseFormData, CourseFilters, CourseResponse } from "@/data/departments";
...
export const useUpdateCourse = () => {
	...
	return useMutation({
		mutationFn: async ({ id, data }: { id: string; data: Partial<CourseFormData> }): Promise<Course> => {
...
export const useToggleCourseStatus = () => {
	...
	return useMutation({
		mutationFn: async ({ id, isActive }: { id: string; isActive: boolean }): Promise<Course> => {
		...
		onSuccess: (data) => {
			queryClient.invalidateQueries({ queryKey: courseKeys.lists() });
			queryClient.invalidateQueries({ queryKey: courseKeys.detail(data.id) });
			toast.success(`Course ${data.isActive ? "activated" : "deactivated"} successfully`);
		},
		onError: (error: any) => {
