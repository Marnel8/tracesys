...
export function InstructorSidebar() {
  ...
  const isMenuItemActive = (item: any): boolean => {
    ...
    if (item.items) {
      ...
      const hasActiveChildItem = item.items.some(
      ...
      if (
        item.title === "Academic Management" &&
        (isCourseEditPage() ||
          isCourseDetailPage() ||
          isDepartmentEditPage() ||
          isDepartmentDetailPage() ||
          isSectionEditPage() ||
          isSectionDetailPage())
      ) {
        return true;
  ...
  const isCourseDetailPage = (): boolean => {
    return (
      pathname.match(/^\/dashboard\/instructor\/courses\/[^\/]+$/) !== null
    );
  };
  ...
  const isDepartmentDetailPage = (): boolean => {
    return (
      pathname.match(/^\/dashboard\/instructor\/departments\/[^\/]+$/) !== null
    );
  };
  ...
  const isSectionDetailPage = (): boolean => {
    return (
      pathname.match(/^\/dashboard\/instructor\/sections\/[^\/]+$/) !== null
    );
  };
                                ...
                                <SidebarMenuSubButton
                                  ...
                                  isActive={
                                    ...
                                    (subItem.title === "Departments" &&
                                      (isDepartmentEditPage() ||
                                        isDepartmentDetailPage())) ||
                                    (subItem.title === "Sections" &&
                                      (isSectionEditPage() ||
                                        isSectionDetailPage())) ||
                                    (subItem.title === "Manage Supervisors" &&
                                      isSupervisorManagementPage())
                                  }
                                  className="w-full"
                                >
                                  <Link
